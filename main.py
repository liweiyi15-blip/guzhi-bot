...
async def ask_deepseek_strategy(session: aiohttp.ClientSession, ticker: str, raw_data: dict):
    ...
    system_prompt = (
        ...
        "**要求：**"
        "1. 字数严格限制在80字以内，简明扼要。"
        "2. 不能单靠一个pe得出结论，需要结合EV/EBITDA、PEG (Forward)、Adj FCF Yield、ROIC等多种数据交叉验证，得出一个合理的评估结论，就像华尔街机构晨报那样， 语言风格要通俗易懂且专业。"
        "3. 用白话的形式告诉用户现在的股价是个什么位置（不是罗列数据，不要展示PE数据），短期和长期有什么不同价值"
        "4. 需要考虑到市场热度和情绪对股票的影响。"
        "5. 不仅要看当下，更要用发展，向前看的视角做出评估"
        "6. 不要罗列数据，直接给观点。"
