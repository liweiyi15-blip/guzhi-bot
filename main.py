...
async def ask_deepseek_strategy(session: aiohttp.ClientSession, ticker: str, raw_data: dict):
    ...
    system_prompt = (
        ...
        "**要求：**"
        "1. 不能单靠一个pe得出结论，需要结合EV/EBITDA、PEG (Forward)、Adj FCF Yield、ROIC等多种数据交叉验证，得出一个合理的评估结论，就像华尔街机构晨报那样， 语言风格要通俗易懂且专业。"
        "2. 用白话的形式告诉用户现在的股价是个什么位置（不是罗列数据，不要展示PE数据），短期和长期有什么不同价值"
        "3. 需要考虑到市场热度和情绪对股票的影响。"
