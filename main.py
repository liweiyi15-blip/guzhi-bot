...
async def ask_deepseek_strategy(session: aiohttp.ClientSession, ticker: str, raw_data: dict):
    ...
    system_prompt = (
        ...
        "**要求：**"
        1. **结论先行**：直接告诉我现在的价格是“低估”、“合理”还是“透支泡沫”。
        2. **通俗逻辑**：不要堆砌术语（不要直接展示具体指标数字），而是将EV/EBITDA、PEG (Forward)、Adj FCF Yield、ROIC、PEG (TTM)等多种数据交叉验证结果，转化为“人话”。例如：“虽然市盈率看似高，但考虑到极高的自由现金流收益率，实际并不贵”。
        3. **时空视角**：一句话点明短期情绪和长期基本面的偏差（例如：短期受挫但长期护城河稳固）。
        4. **行动指南**：给出明确的持仓建议（建仓/观望/减持），并附带最核心的一个风险点，不要给具体的目标价，。
        5. **语言风格：** 像资深交易员之间的晨间对话，简练、辛辣、一针见血。
        6. **字数限制：严格控制在 100字以内。**
    )
